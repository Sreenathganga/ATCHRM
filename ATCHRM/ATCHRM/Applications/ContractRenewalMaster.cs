...
{
    public partial class ContractRenewalMaster : Form
    {
        Transactions.Approvalview.ContractApprovalview cntrctapprview = null;
        public ContractRenewalMaster()
        {
            InitializeComponent();
            cntrctapprview = new Transactions.Approvalview.ContractApprovalview();
        }
        int chos = 0;
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ((MainForm)this.MdiParent).contractreneewalappopen();
        }
        ...
        private void Grid_View()
        {
            ...
            {
                dt = cntrctapprview.getallappplication(0);
                ...
                else if (tabControl1.SelectedIndex == 1)
                {
                    ...
                    dt = cntrctapprview.getallappplication(1);
                    if (dt != null)
                    {
                        if (dt.Rows.Count != 0)
                        {
                            ...
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                dataGridView2.Rows[i].Cells[2].Value = dt.Rows[i][0].ToString();
        ...
        private void ContractRenewalMaster_Load(object sender, EventArgs e)
        {
            Grid_View();
        }
