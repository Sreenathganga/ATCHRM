...
{
    public partial class DesignationChangeMaster : Form
    {
        Transactions.Approvalview.DesignChangeView cntrctapprview = null;
        public DesignationChangeMaster()
        {
            InitializeComponent();
            cntrctapprview = new Transactions.Approvalview.DesignChangeView();
        }

        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ((MainForm)this.MdiParent).designationchangformOpen(); 
        }
        ...
        private void Grid_View()
        {
            ...
            {
                dt = cntrctapprview.getAllDesgChangeApp (0);
            ...
            else if (tabControl1.SelectedIndex == 1)
            {
                ...
                dt = cntrctapprview.getAllDesgChangeApp(1);
                if (dt != null)
                {
                    if (dt.Rows.Count != 0)
                    {
        ...
        int chos = 0;
        private void DesignationChangeMaster_Load(object sender, EventArgs e)
        {
            Grid_View();
        }
