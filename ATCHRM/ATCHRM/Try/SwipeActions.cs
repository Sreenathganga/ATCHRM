...
namespace DeviceManager
{
    public class SwipeActions : IDisposable
    {
        ...
        public String con = Connectionstring.ConnectionString;
        public void Dispose()
        {
            if (Connection != null)
            {
            ...
            if (ifSwipedidntexist(EmployeeID, SwipedTime))
            {
                ...
                else if (SwipeType == "Out")
                {
                    if (Out_abscent(EmployeeID, Swipedate))
                    {
                        ...
                        //if (ShiftPk == "68")
                        //{
                        //    cmd.Parameters.Add(new SqlParameter("@Date", SqlDbType.Date)).Value = Swipedate.Date.AddDays(-1);
                        //}else
                        //{
                           cmd.Parameters.Add(new SqlParameter("@Date", SqlDbType.Date)).Value = Swipedate.Date;
                        //}
        ...
        public Boolean Out_abscent(string empid, DateTime swipedate)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //if (ShiftPk == "68")
                    //{
                    //    cmd.Parameters.Add(new SqlParameter("@date", SqlDbType.DateTime)).Value = swipedate.Date.AddDays(-1);
                    //}
                    //else
                    //{
