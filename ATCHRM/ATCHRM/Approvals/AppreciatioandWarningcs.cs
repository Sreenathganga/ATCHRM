...
{
    public partial class AppreciatioandWarningcs : Form
    {
        int ActionNum = 0;
        Transactions.ApprovalTransaction apprvltrans = null;
        public AppreciatioandWarningcs()
        {
            InitializeComponent();
        }

        public AppreciatioandWarningcs(int actionnum)
        {
            InitializeComponent();
            ActionNum = actionnum;
        ...
        public void filldatagridview()
        {
            ...
            DataTable dt = apprvltrans.getAllEmployeeAppreciation(Program.LOCTNPK, ActionNum, "Apreciation");
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            try
            {
                for (int i = 0; i < tbl_Approval.Rows.Count; i++)
                {
                    ...
                    {
                        apprvltrans.updateAppreciation(int.Parse(tbl_Approval.Rows[i].Cells[1].Value.ToString()), ActionNum);
            ...
            try
            {
                for (int i = 0; i < tbl_Approval.Rows.Count; i++)
                {
                    if (Convert.ToBoolean(tbl_Approval.Rows[i].Cells[0].Value) == true)
                    {
                        ...
                        apprvltrans.rejectGatepass(int.Parse(tbl_Approval.Rows[i].Cells[1].Value.ToString()), ActionNum);
                    }
                }
