...
namespace ATCHRM.Transactions
{
    class AnualContractrenewal
    {
        ...
        public void insertContractSubdetails( DataTable empcontractdata, DataTable empcntrctsubdata)
        {
            ...
            {
                //changed by sreenath
                maxLavel = Convert.ToDateTime(empcntrctsubdata.Compute("max(ToDate)", string.Empty));
                newfromdate = Convert.ToDateTime(empcontractdata.Compute("max(Startdate)", string.Empty));
