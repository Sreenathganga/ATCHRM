...
//        }
        ...
        public String isdesertee(int empid)
        {
            ...
            using (SqlServerLinqDataContext cntxt= new SqlServerLinqDataContext (Program.ConnStr))
            {
                ...
                {
                    //deserteestatus = (element.IsDesertee.ToString().Trim() == "") ? "N" : element.IsDesertee.ToString().Trim();
        ...
        public void insertemployeePayrollDetails(Datalayer.PayrollEmployeeDetailsDataBean  payEmpDetailDataBean )
        {
            ...
            try
            {
                ...
                basic = getbasicofemployee(arry, paydaysasper26);
            ...
            try
            {
                ...
                //  specialallowance = (specialallowance / 26) * (dayadiff-payEmpDetailDataBean.Offdays);
          ...
          //  holidays = gettheholidaysforemployeeincontract(int.Parse(arry[1].ToString()), fromdate, todate, dayadiff);
            ...
            try
            {
                ...
                using (SqlConnection sqlConnection1 = new SqlConnection(Program.ConnStr))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("insert into Payroll_PayDetails (pay_id,emp_id,total_salary,Disburs_Amnt,Deduct_Amnt,leave_days,absent_no,ot_amnt,Special_Allow,holidayno,offdays,Payrolldays ,ActualBasic ,PerkAmount,PerkTaxable,LeaveOnOff,PaidLeaves,halfdayleave,halfdayAbscent) " +
" values(@Param1,@Param2,@Param3,@Param4,@Param5,@Param6,@Param7,@Param8,@Param9,@Param10 ,@Param11,@Param12,@Param13,@Param14,@Param15,@Param16,@Param17,@Param18,@Param19) ;SELECT SCOPE_IDENTITY() ", sqlConnection1))
                    {
                    ...
                    using (SqlCommand command = new SqlCommand("	UPDATE    Payroll_PayDetails " +
" SET              Basic_Sal = @Param1, HRA_Allow = @Param2, NSSF = @Param3, NHIF = @Param4, Ot_Hrs = @Param5, OfOt_Hrs = @Param6,Normal_OTHours=@Param7,AdvanceToPay=@Param8 "  +
" WHERE     (Emp_Id = @empid) AND (Pay_Id = @payid) AND (Pay_Det_Id = @paydetid)", sqlConnection1))
                    {
                        command.Parameters.AddWithValue("@Param1", basic);
                        command.Parameters.AddWithValue("@Param2", hra);
...
WHERE        (Pay_Id = @payid)", sqlConnection1))
        ...
        /// <param name="dayadiff"></param>
        /// <returns></returns>
        public float  getbasicofemployee(ArrayList arry, int dayadiff)
        { 
            float  basic = 0;
            int employeeworkdays = 0;
            int offdaysinperiod=0;
